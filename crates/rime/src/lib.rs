...
macro_rules! api {
    ($api:expr, $f:ident) => {
        $api.$f.expect(concat!("librime API missing ", stringify!($f)))
    };
}
...
impl Default for Config {
    fn default() -> Self {
        ...
        Config {
            ...
            user_data_dir: data_home.join("popeinput").join("rime"),
            schema: None,
            page_size: 9,
        }
    }
...
fn init_library(cfg: &Config) -> Result<Library, String> {
    ...
    traits.data_size = (std::mem::size_of::<sys::RimeTraits>() - std::mem::size_of::<c_int>()) as c_int;
    ...
    let lib = Library { api, _strings: strings };
...
fn ensure_default_custom(cfg: &Config) {
    ...
    ids.sort_by_key(|id| !(id.starts_with("cangjie") || id.starts_with("quick") || id.contains("jyut")));
    let mut yaml = String::from(
        "# Generated by popeinput on first run. Edit freely; it is never overwritten.\n\
    ...
    yaml.push_str(&format!("  menu/page_size: {}\n", cfg.page_size.clamp(1, 9)));
...
fn list_schemas(lib: &Library) -> Vec<Schema> {
    let mut out = Vec::new();
    let mut list = sys::RimeSchemaList { size: 0, list: std::ptr::null_mut() };
...
impl RimeEngine {
    pub fn new(cfg: Config) -> Result<Self, String> {
        ...
        let mut engine = RimeEngine { lib, session, snapshot: Snapshot::default() };
        ...
        engine.refresh();
        log::info!("RIME session ready, schema {:?}", engine.snapshot.schema_name);
        Ok(engine)
    }
    ...
    fn refresh(&mut self) {
        ...
        unsafe {
            ...
            status.data_size = (std::mem::size_of::<sys::RimeStatus>() - std::mem::size_of::<c_int>()) as c_int;
            ...
            ctx.data_size = (std::mem::size_of::<sys::RimeContext>() - std::mem::size_of::<c_int>()) as c_int;
    ...
    fn take_commit(&mut self) -> Option<String> {
        ...
        unsafe {
            ...
            commit.data_size = (std::mem::size_of::<sys::RimeCommit>() - std::mem::size_of::<c_int>()) as c_int;
